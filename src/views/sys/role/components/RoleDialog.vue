    ...
    <el-dialog v-el-drag-dialog :title="dialog.title" :visible.sync="dialog.show" width="500px" @close="closeDialog">
      <el-form ref="form" :model="dialog.form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="角色编码" prop="number">
          <el-input v-model="dialog.form.number" placeholder="请输入角色编码" />
        </el-form-item>
        ...
        <el-form-item label="数据权限">
          <el-select v-model="dialog.form.dataScope" placeholder="数据权限" clearable size="small">
            <el-option
              v-for="dict in dialog.dictValues['dataScope']"
              :key="dict.key"
              :label="dict.value"
              :value="dict.key"
            />
...
import { insertRole, updateRole, leftTreeData } from '@/api/sys/role' // 角色api
export default {
  name: 'RoleDialog', // 组件名称
  components: {
    // 注册树形结构选择器
    Treeselect
  },
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (model.id) {
            updateRole(model.id, this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '修改角色成功',
                type: 'success'
            ...
            insertRole(this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '新增角色成功',
                type: 'success'
