  ...
  <div>
    ...
    <el-dialog v-el-drag-dialog :title="dialog.title" :visible.sync="dialog.show" width="500px" @close="closeDialog">
      <el-form ref="form" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="表信息编码" prop="number">
          <el-input v-model="dialog.form.number" placeholder="请输入表信息编码" />
        </el-form-item>
        <el-form-item label="表信息名称" prop="name">
          <el-input v-model="dialog.form.name" placeholder="请输入表信息名称" />
        </el-form-item>
        <el-form-item label="表信息状态" prop="enabled">
...
import { insertTableInfo, updateTableInfo } from '@/api/tool/tableInfo' // 表信息api
export default {
  name: 'TableInfoDialog', // 组件名称
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '表信息名称不能为空', trigger: 'blur' }
        ],
        number: [
          { required: true, message: '表信息编码不能为空', trigger: 'blur' }
        ]
      }
    }
  },
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (model.id) {
            updateTableInfo(model.id, this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '修改表信息成功',
                type: 'success'
            ...
            insertTableInfo(this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '新增表信息成功',
                type: 'success'
