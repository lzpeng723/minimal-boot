  ...
  <div>
    ...
    <el-dialog v-el-drag-dialog :title="dialog.title" :visible.sync="dialog.show" width="500px" @close="closeDialog">
      <el-form ref="form" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="方法日志编码" prop="number">
          <el-input v-model="dialog.form.number" placeholder="请输入方法日志编码" />
        </el-form-item>
        <el-form-item label="方法日志名称" prop="name">
          <el-input v-model="dialog.form.name" placeholder="请输入方法日志名称" />
        </el-form-item>
        <el-form-item label="方法日志状态" prop="enabled">
...
import { insertMethodLog, updateMethodLog } from '@/api/monitor/methodLog' // 方法日志api
export default {
  name: 'MethodLogDialog', // 组件名称
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '方法日志名称不能为空', trigger: 'blur' }
        ],
        number: [
          { required: true, message: '方法日志编码不能为空', trigger: 'blur' }
        ]
      }
    }
  },
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (model.id) {
            updateMethodLog(model.id, this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '修改方法日志成功',
                type: 'success'
            ...
            insertMethodLog(this.filterModel(model)).then(res => {
              this.$notify({
                title: '成功',
                message: '新增方法日志成功',
                type: 'success'
