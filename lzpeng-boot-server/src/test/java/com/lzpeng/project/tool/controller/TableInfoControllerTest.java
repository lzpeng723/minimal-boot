...
public class TableInfoControllerTest {
    ...
    public void testSave() throws Exception {
        ...
        String content = objectMapper.writeValueAsString(tableInfo);
        String result = mockMvc.perform(post("/tool/tableInfo")
...
//                .contentType(MediaType.APPLICATION_FORM_URLENCODED) // @RequestParam 解析
    ...
    public void testDelete() throws Exception {
        String id = "{}";
        mockMvc.perform(delete("/tool/tableInfo/1" + id)
                .header(HttpHeaders.AUTHORIZATION, String.join(" ", accessToken.getTokenType(), accessToken.getValue()))
    ...
    public void testUpdate() throws Exception {
        ...
        TableInfo tableInfo = new TableInfo();
        String content = objectMapper.writeValueAsString(tableInfo);
        String result = mockMvc.perform(put("/tool/tableInfo/" + id)
                .header(HttpHeaders.AUTHORIZATION, String.join(" ", accessToken.getTokenType(), accessToken.getValue()))
                .content(content) // @RequestBody 解析
    ...
    public void testQuery() throws Exception {
        ...
        TableInfo tableInfo = new TableInfo();
        String content = objectMapper.writeValueAsString(tableInfo);
        String result = mockMvc.perform(get("/tool/tableInfo/"+ page + "/" + size)
                .header(HttpHeaders.AUTHORIZATION, String.join(" ", accessToken.getTokenType(), accessToken.getValue()))
                .content(content)
                .contentType(MediaType.APPLICATION_JSON_UTF8) // 请求类型
                .accept(MediaType.APPLICATION_JSON_UTF8)) // 响应类型
    ...
    public void testFindById() throws Exception {
        String id = "{}";
        String result = mockMvc.perform(get("/tool/tableInfo/"+ id)
                .header(HttpHeaders.AUTHORIZATION, String.join(" ", accessToken.getTokenType(), accessToken.getValue()))
                .contentType(MediaType.APPLICATION_JSON_UTF8) // 请求类型
                .accept(MediaType.APPLICATION_JSON_UTF8)) // 响应类型
                .andExpect(status().isOk())
